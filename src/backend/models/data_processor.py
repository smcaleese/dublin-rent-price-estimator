...
class DataProcessor:
    def __init__(self):
        self.df = None
        self.prop_type_encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
        self.dublin_area_encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    ...
    def _clean_data(self):
        ...
        self.df = self.df.dropna(subset=['price', 'beds', 'baths', 'prop_type', 'dublin_area'])
    ...
    def prepare_features(self):
        ...
        dublin_area_encoded = self.dublin_area_encoder.fit_transform(self.df[['dublin_area']])
        
        # Create feature names
        prop_type_features = [f"prop_type_{cat}" for cat in self.prop_type_encoder.categories_[0]]
        dublin_area_features = [f"dublin_area_{int(cat)}" for cat in self.dublin_area_encoder.categories_[0]]
        ...
        # Store feature names
        self.feature_names = ['beds', 'baths'] + prop_type_features + dublin_area_features
    ...
    def get_property_types(self):
        ...
        if hasattr(self.prop_type_encoder, 'categories_') and len(self.prop_type_encoder.categories_) > 0:
    ...
    def get_dublin_areas(self):
        ...
        if hasattr(self.dublin_area_encoder, 'categories_') and len(self.dublin_area_encoder.categories_) > 0:
            return sorted([int(area) for area in self.dublin_area_encoder.categories_[0]])
        return []
